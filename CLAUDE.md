...
- **底色深綠、主色亮橘、頁尾淺綠**（色層少、降低填充）：底 `#0F1712`、主色 `--accent #FF8A3C`、頁尾 `#C9D8A6`。亮橘上一律**深字 `#1A1005`**（白字對比不足）。
- **卡片透明**（露出底色綠 + 格線），只有必要面板才填色。**區塊本身無 hover 特效**。
- **像素 / retro**：結構容器直角、硬陰影；**按鈕與 chips 用 4px 圓角**；chips **實心無框**且文字前加亮橘 `#`（CSS `::before`）。像素細節：亮橘角釘、44px 格線畫布、像素虛線、長「>」箭頭 / 粗漢堡 / 像素信封圖示。
- **字體**：內文黑體 Noto Sans TC；**大標題用中文像素字 Zpix**（哩賀! / 我是 UMIN / 感謝您的時間! / 歡迎常來坐!，經 jsDelivr `@font-face` 載入）；年份 / 步驟編號用 Press Start 2P。內文數字不用像素。
- **頁尾**：淺綠斷差區塊、加大高度、頂端做出「滑到下一段」的斷差感；`聯絡我` 用像素信封、`LinkedIn` / `前往 VIP 特區` 用 `.btn--dark`。
