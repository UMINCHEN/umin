...
- **像素 / retro**：結構容器直角、卡片硬陰影；**按鈕與 chips 都用 4px 圓角**；**按鈕用像素字體且無陰影**；chips **透明無底色、無框、字級與內文一致**、文字前加**白色 `#`**（CSS `::before`）。像素細節：**角釘 `--stud = --surface-2`**、44px 格線畫布、長「>」箭頭 / 簡單粗漢堡（無外框）/ 像素信封圖示。**站內不使用虛線**。
