...
- **像素 / retro**：結構容器直角、卡片硬陰影；**按鈕與 chips 用 4px 圓角**；**按鈕用像素字體且無陰影**；chips **實心無框**、文字前加**白色 `#`**（CSS `::before`）。像素細節：**極淺 sage 角釘 `--stud #E7EBD2`**、44px 格線畫布、長「>」箭頭 / 簡單粗漢堡（無外框）/ 像素信封圖示。**站內不使用虛線**。
- **字體**：內文黑體 Noto Sans TC；**大標題、導航列、卡片標題、彈窗標題都用中文像素字 Zpix**（哩賀! / 我是 UMIN / 感謝您的時間! / 歡迎常來逛逛…，經 jsDelivr `@font-face` 載入）；**按鈕也用 Zpix**；年份 / 步驟編號用 Press Start 2P。內文與年份數字不用陰影。
- **頁尾**：**黑底 `#07080B` + 淡像素星空**、加大高度，做出「滑到下一段」的斷差感；`id-line` 用主色橘、`聯絡我` 用像素信封、`LinkedIn` / `前往 VIP 特區` 用預設按鈕。
- **彈窗**：內容**置中對齊**；`modal__kicker` 顯示**專案名（不含年份）**；`.id-line` 與 `case__h` 用主色橘；`哩賀!` 為白字。`了解更多`＝橘色實心 primary、`前往站台`＝預設框線。
