...
- **圓角**：結構容器直角；chips 用 4px（`--radius`）；**按鈕全圓角**（`--radius-pill: 999px`，唯一例外）
...
## 5. 元件 Components
- **按鈕 `.btn`**：**像素字體（Zpix）、無陰影**、2px 邊 + **全圓角（`--radius-pill` 999px）** + 按壓位移。`--primary`＝亮橘填色 + 深字；預設＝深綠面板 + 骨白字；`--sm`＝導航用。`.btn-icon`＝彈窗關閉圓鈕（同全圓角）。文字級距：一般 1rem／`--sm` 0.85rem。
- **chips `.tag`**：**透明無底色、無框**、4px 圓角、文字前用 CSS `::before` 加**白色 `#`**（`--ink`；HTML 不寫 #，避免重複）。
...
- **專案**：長方卡片、圖文左右交錯（`.work-card` / `--flip`）；卡片透明只留邊框 + 角釘；**標題下方即接 chips**，接著才是描述與按鈕；**有對應案例彈窗的專案，年份移到彈窗白色標題內**（`.modal__title-year`，Zpix、`--ink-mid`、比標題小一級），卡片本身不再顯示年份；**沒有彈窗的專案（例如純外部連結案例）年份留在卡片標題旁**。卡片標題放大加浮凸；**內文段落用白字 `--ink`**；`≤860px` 單欄。**無區塊 hover 特效**。
