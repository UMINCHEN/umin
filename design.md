...
- `--font-pixel-cjk: "Zpix"` - **只用在大標題**：`哩賀!` / `我是 UMIN` / `感謝您的時間!` / `歡迎常來坐!`。Zpix 為向量像素字、放大不糊，含中英；透過 jsDelivr 載入（`@font-face`），失敗則 fallback 到 Noto Sans TC。
...
## 4.5 像素風細節（內文維持黑體）
- **角釘 `--corner-studs`**：卡片與 core 四角的 6px 亮橘小方塊（單一 `::after` + 四角漸層）。
...
## 5. 元件 Components
- **按鈕 `.btn`**：直角 2px 邊 + 4px 圓角 + 硬陰影 + 按壓位移。`--primary`＝亮橘填色 + 深字；預設＝深綠面板 + 骨白字；`--dark`＝深綠填色 + 骨白字（用於淺綠頁尾）；`--sm`＝導航用。`.btn-icon`＝彈窗關閉方鈕。
- **chips `.tag`**：**實心無框**（`--surface-2` 填色）、4px 圓角、文字前用 CSS `::before` 加亮橘 `#`（HTML 不寫 #，避免重複）。
...
- **全螢幕彈窗 `.modal`**：滿版、不跳頁；Esc / 點背景 / 關閉鈕關閉；`.case__h`（左橘邊標題）、`.step`（像素編號）。
- **頁尾 `.foot`**：**淺綠斷差區塊**，深色文字、加大高度（`128–140px`）、頂端 5px 深色邊 + 亮橘像素虛線做出「滑到下一段」的斷差感。

## 6. 版面規則
- **Hero**：左右分割（文字左、形象照右）；手機單欄且**形象照 fill 滿版寬**。
- **專案**：長方卡片、圖文左右交錯（`.work-card` / `--flip`）；卡片透明只留邊框 + 角釘；`≤860px` 單欄。**無區塊 hover 特效**。
