...
- **底色 = 深綠**；**主色（唯一強調色）= 亮橘**；**頁尾 = 深色區塊**（頂端亮橘像素虛線做斷差）
...
- `--font-pixel-cjk: "Zpix"` - **用在大標題與所有按鈕文字**：`哩賀!` / `我是 UMIN` / `感謝您的時間!` / `歡迎常來坐!` 等大標題，以及 `.btn`。Zpix 為向量像素字、放大不糊，含中英；透過 jsDelivr 載入（`@font-face`），失敗則 fallback 到 Noto Sans TC。
...
## 4.5 像素風細節（內文維持黑體）
- **角釘 `--corner-studs`**：卡片與 core 四角的 6px **sage 綠**小方塊 `--stud #B0B87A`（非橘；單一 `::after` + 四角漸層）。
...
- **頁尾 `.foot`**：**深色區塊（原版色 `--surface`）**、骨白文字、加大高度（約 `120–130px`）、頂端亮橘像素虛線做出「滑到下一段」的斷差感。
