...
## 5. 元件 Components
- **按鈕 `.btn`**：**像素字體（Zpix）**、直角 2px 邊 + 4px 圓角 + **硬陰影（Windows 98 式色塊陰影，無模糊）`3px 3px 0`，陰影色＝該按鈕自己的邊框色**（`--btn-bd`，深色底上才看得見，不用統一的深色 rgba，否則深底上會幾乎隱形）；按下時位移 3px 抵銷陰影，模擬實體按壓。`--primary`＝亮橘填色 + 深字（邊框/陰影 `--accent-deep`）；預設＝深綠面板 + 骨白字（邊框/陰影 `--line`）；`--sm`＝導航用。`.btn-icon`＝彈窗關閉方鈕（陰影 `--line`）。文字級距：一般 1rem／`--sm` 0.85rem。
...
- **專案**：長方卡片、圖文左右交錯（`.work-card` / `--flip`）；卡片透明只留邊框 + 角釘；**標題下方即接 chips**，接著才是描述與按鈕；**有案例彈窗的專案，年份放在彈窗白色標題內、靠右對齊**（`.modal__title` 用 flex + `space-between`，年份 `.modal__title-year` 用 Zpix、`--ink-mid`、比標題小一級，固定貼右上）；卡片本身不重複顯示年份。沒有彈窗的專案（例如純外部連結案例）目前不顯示年份。卡片標題放大加浮凸；**內文段落用白字 `--ink`**；`≤860px` 單欄。**無區塊 hover 特效**。
